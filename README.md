<h1 align="center">Hi, I'm Wasif 👋</h1>

<p align="center">
  <b>Fullstack Developer · AI Engineer · Automation Builder</b><br>
  Shipping production web apps, agentic AI systems, and workflow automations from Dhaka 🇧🇩
</p>

<p align="center">
  <a href="mailto:wasifahad300@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white"></a>
  <a href="https://www.linkedin.com/in/wasifahad/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white"></a>
  <a href="https://www.wasif23ahad.me/"><img src="https://img.shields.io/badge/Portfolio-22C55E?style=flat&logo=googlechrome&logoColor=white"></a>
  <a href="https://x.com/WasifAhad1"><img src="https://img.shields.io/badge/X-000000?style=flat&logo=x&logoColor=white"></a>
  <a href="https://www.facebook.com/wasifahad23"><img src="https://img.shields.io/badge/Facebook-1877F2?style=flat&logo=facebook&logoColor=white"></a>
  <a href="https://www.instagram.com/wasif23ahad/"><img src="https://img.shields.io/badge/Instagram-E4405F?style=flat&logo=instagram&logoColor=white"></a>
</p>

---

### About

I build end-to-end products — from full-stack web apps with auth, payments, and multi-role dashboards, to RAG pipelines, agentic LLM systems, and n8n-powered automations that replace manual work.

- 🎓 MSc in Computer Science & Engineering @ **BRAC University**
- 🔧 Co-founder @ **[SimpliDigify](https://simplidigify.com)** — AI automation & web dev agency
- 🛒 Founder @ **[Abstract Goodies](https://abstractgoodies.com)** — e-commerce gift shop
- 💼 **Open to roles:** Fullstack · AI Engineering · Backend · Automation (remote or Dhaka)

---

### Tech Stack

**Frontend** — Next.js · React · TypeScript · Tailwind · shadcn/ui

**Backend** — Node.js · Express · FastAPI · Prisma · PostgreSQL · MongoDB · BetterAuth · Stripe

**AI / ML** — Python · LangChain · FAISS · HuggingFace · OpenAI · Anthropic · PyTorch · YOLOv8

**Automation & Infra** — n8n · whatsapp-web.js · Docker · DigitalOcean · Cloudinary · ngrok

---

### Featured Projects

🍽️ **FoodHub** — Multi-role food delivery platform
Customer, restaurant, rider, and admin flows with cross-domain cookie auth.
`Next.js 15` `React 19` `Express` `Prisma` `PostgreSQL` `BetterAuth` `Cloudinary`

🎟️ **Planora** — Full-stack events management platform with Stripe ticketing.
`Next.js` `Express` `Prisma` `PostgreSQL` `Stripe`

🤖 **RAG Document Chatbot** — Chat with your PDFs, end to end.
`FastAPI` `LangChain` `FAISS` `HuggingFace`

📲 **WhatsApp Bulk Messaging Automation** — Catalog blasts for Abstract Goodies.
`n8n` `whatsapp-web.js` `ngrok`

📈 **Reflective LLM Trading Agent** — Self-reflective LLM agent for BTC/ETH market analysis.
`Python` `LangChain` `LLM Agents`

🔬 **UVM Testbench (4-bit Adder)** — Verification with SystemVerilog + UVM.
`SystemVerilog` `UVM` `QuestaSim` `Cadence Xcelium`

---

### Currently

- 📚 Researching FPGA-based AI accelerators (MSc thesis direction)
- 🛠️ Building agentic systems with structured AGENTS.md / SKILLS.md workflows

---

### Get in Touch

<p>
  📧 <a href="mailto:wasifahad300@gmail.com">wasifahad300@gmail.com</a> &nbsp;·&nbsp;
  🌐 <a href="https://www.wasif23ahad.me/">wasif23ahad.me</a> &nbsp;·&nbsp;
  💼 <a href="https://www.linkedin.com/in/wasifahad/">LinkedIn</a> &nbsp;·&nbsp;
  🐦 <a href="https://x.com/WasifAhad1">X</a> &nbsp;·&nbsp;
  📘 <a href="https://www.facebook.com/wasifahad23">Facebook</a> &nbsp;·&nbsp;
  📸 <a href="https://www.instagram.com/wasif23ahad/">Instagram</a> &nbsp;·&nbsp;
  💬 <a href="https://wa.me/8801973116555">WhatsApp</a>
</p>
<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=wasif23ahad&theme=tokyonight" height="160" alt="Repos Per Language">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=wasif23ahad&theme=tokyonight" height="160" alt="Most Commit Language">
</p>
